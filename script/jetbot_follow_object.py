...
def detect_callback(msg):
		...
		set_speed(motor_right_ID, 0.0) 
            ...
            for detection in msg.detections:
                    square = detection.bbox.size_x * detection.bbox.size_y
                    if square > biggest_box:
                            biggest_box = square
                            biggest_detect = detection
            bbox_x = biggest_detect.bbox.center.x
            bbox_y = biggest_detect.bbox.center.y
            bbox_height = biggest_detect.bbox.size_x
            rospy.loginfo("x:%d, y:%d, height:%d", bbox_x, bbox_y, bbox_height)
            speed = (bbox_x - 640)/1000
            if bbox_x > 1280/2 + 200:
	    	set_speed(motor_left_ID,   -0.5)
	    	set_speed(motor_right_ID,   0.5) 
            elif bbox_x < 1280/2 - 200:
	    	set_speed(motor_left_ID,   0.5)
	    	set_speed(motor_right_ID, -0.5) 
...
if __name__ == '__main__':
	...
	rospy.Subscriber('/detectnet/detections', Detection2DArray, detect_callback)
