...
def on_cmd_dir(msg):
	...
	Data = msg.data
	Degree,Speed = Data.split(',')
	Degree = float(Degree) # positive : forward, negative : backward
	Speed = float(Speed) # positive : right, negative : left 

	Left_Gain = 0.79
	Right_Gain = 0.85
	
	if Degree == 0 :	
		set_speed(motor_left_ID,   Speed * 0.78)
		set_speed(motor_right_ID,   Speed * 1.0)
	elif Degree < 0 :
		set_speed(motor_left_ID,  -1.0)
		set_speed(motor_right_ID,  1.0) 
		rospy.sleep(Left_Gain * Degree/180)
		all_stop()
	else :
		set_speed(motor_left_ID,   1.0)
		set_speed(motor_right_ID, -1.0) 
		rospy.sleep(Right_Gain * Degree / 180)
		all_stop()
